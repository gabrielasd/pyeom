"""Title: $title"""
import os

import numpy as np

import pyci
from iodata import load_one

from eomee.excitation import EOMExc
from eomee.tools import spinize, hartreefock_rdms


def run_acph(NAME, operator, solver, eigtol):
    # Get electron integrals in MO format
    print('Load Hamiltonian')
    if not os.path.isfile(f'{NAME}.FCIDUMP'):
        raise ValueError(f'{NAME}.FCIDUMP not found')
    ham = pyci.hamiltonian(f"{NAME}.FCIDUMP")
    one_mo = ham.one_mo 
    two_mo = ham.two_mo
    
    print('Load Molden file')
    if not os.path.isfile(f"{NAME}.molden"):
        raise ValueError(f'f"{NAME}.molden" not found')
    mol = load_one(f"{NAME}.molden")
    nelec = int(mol.nelec)
    na = nelec // 2
    nb = nelec // 2
    rdm1, rdm2 = hartreefock_rdms(ham.nbasis, na, nb)

    # Evaluate ERPA (particle-hole)
    print('Run ERPA (particle-hole)')
    if operator != 'ph':
        raise ValueError('Invalid operator.')
    h = spinize(one_mo) 
    v = spinize(two_mo)
    erpa = EOMExc(h, v, rdm1, rdm2)
    w, cv = erpa.solve_dense(mode=solver, tol=eigtol)
    
    np.savez(f"{NAME}.erpa{operator}{solver}.npz", energies=w, coeffs=cv)
    print('')


NAME = '$output'
CHARGE = $charge
MULT = $spinmult
eigtol = 1.0e-7


run_acph(NAME, 'ph', 'qtrunc', eigtol) # 'nonsymm'
