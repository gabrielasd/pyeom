...
def casscf_initial_guess(guess_dir, f_name):
    gname = f'{guess_dir}/{f_name}/{f_name}'
    mol, mo_energy, mo_coeff, mo_occ, irrep_labels, spins = molden.load(f"{gname}.molden")
    mf = mol.RHF()
    dm_init_guess = mf.make_rdm1(mo_coeff, mo_occ)
    data = np.load(f"{gname}.cas.npz")
    mo_coeff = data['mocoeff']
    ci = data['ci']    
    return dm_init_guess, mo_coeff, ci


def run_casscf(atcoords, charge, mult, basis, fname, gname, unit='B'):
    ...
    dm = mo = ci = None
    if gname != "":
        dm, mo, ci = casscf_initial_guess(gname, fname)
    hf = scf.RHF(mol)
    hf.conv_tol = 1.0e-12
    hf.kernel(dm)
    ...
    np.savez(f"{fname}.cas.npz", energy=mc.e_tot, ecore=energy_core, mocoeff=c, ci=mc.ci)
...
# Run PySCF
run_casscf(GEOM, CHARGE, MULT, BASIS, NAME, GUESS, unit='A')
# run_casscf(NAME)
